Shader "Unlit/PhotoFrame"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _ShadeOffset ("ShadeOffset", Vector) = (0.5, 0.5, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float border_radius_alpha(float2 uv) {
                float2 folded_uv = float2(
                    uv.x > 0.5 ? 1 - uv.x : uv.x,
                    uv.y > 0.5 ? 1 - uv.y : uv.y
                );
                ...
                return step(1, radius + u_area + v_area);
            }
